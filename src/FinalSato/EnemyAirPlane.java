...
public class EnemyAirPlane implements Enemy
{
	...
	// EnemyAirPlane class HAS-A 2 double variable store how fast it move upward
	// and downward
	private double inertia, rate;

	// EnemyAirPlane class HAS-A 3 int to store how fast it move and range of
	// their movement on Y-axis
	private int accel, heightInertiaMax, heightInertiaMin;

	// EnemyAirPlane class HAS-A Random object
	private Random random = new Random();

	/**
	 * Purpose: Constructor for the EnemyAirPlane class
	 * 
	 * @param image The image of the enemy airplane.
	 * @param index The index of the enemy airplane.
	 * 
	 */

	public EnemyAirPlane(BufferedImage image, int index)
	{
		this.image = image;
	...
	public void appear()
	{
		accel = random.nextInt(4, 8);
		thisX = 620;
		thisY = (int) (Math.random() * 400);
		waittime = waittime + (int) (Math.random() * 20 * indexOfThis);
		heightInertiaMax = random.nextInt(3, 8);
		heightInertiaMin = -random.nextInt(3, 8);
		rate = random.nextDouble(0.2, 1);
	}
	...
	public void drawing(Graphics g, ImageObserver io)
	{
		...
		// if rising for this enemy is true, rise its altitude by rate,
		if (rising == true)
		{
			inertia -= rate;
			// if rising for this enemy is false, rise its altitude by rate,
		}
		if (rising == false)
		{
			inertia += rate;
		}
		// if inertia of this enemy reach its max or min, Reverses the
		// true/false
		if (inertia > heightInertiaMax || inertia < heightInertiaMin)
		{
			rising = !rising;
		}
		// set recent X and Y coodinate of this enemy
		thisY -= inertia;
		thisX -= (8 + accel);
	...
	public boolean enemyCollision(int[] pCollision)
	{
		...
		// Check if the enemy airplane collides with player
